...
weights.push(
  Deeplearn.variable(
    Deeplearn.randomNormal(
      [INPUT_DIMENSIONS_NUMBER + HIDDEN_VARIABLES_NUMBER, MAT_WIDTH],
      0,
      WEIGHTS_STDEV
    )
  )
...
for (let i = 0; i < MAX_LAYERS; i++) {
  weights.push(
    Deeplearn.variable(
      Deeplearn.randomNormal([MAT_WIDTH, MAT_WIDTH], 0, WEIGHTS_STDEV)
    )
  );
}
weights.push(
  Deeplearn.variable(
    Deeplearn.Array2D.randTruncatedNormal([MAT_WIDTH, 4], 0, WEIGHTS_STDEV)
  )
);
