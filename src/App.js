...
export default class App {
  constructor() {
    ...
    Util.getImage("profile-22.jpg")
      ...
      .then((buffer2) => {
        ...
        this.addLatentVariablesShader = Util.getAddLatentVariablesShader(
          this.gpgpu,
          INPUT_DIMENTIONS_NUMBER
        );

        this.weights = [];

        this.weights.push(
          Deeplearn.Array2D.randTruncatedNormal(
            [this.MAT_WIDTH, INPUT_DIMENTIONS_NUMBER + HIDDEN_VARIABLES_NUMBER],
            0,
            WEIGHTS_STDEV
          )
        );
        ...
        this.weights.push(
          Deeplearn.Array2D.randTruncatedNormal(
            [3 /** max output channels */, this.MAT_WIDTH],
            0,
            WEIGHTS_STDEV
          )
        );
  ...
  getOutput(prevOutput) {
    ...
    const inputWithLatentVariables = Deeplearn.NDArray.make(
      this.input.shape,
      {}
    );
